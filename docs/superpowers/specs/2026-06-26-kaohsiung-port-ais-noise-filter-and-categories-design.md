...
**全 551 目標中,110 個(20%)是非船 / 雜訊**,目前全被畫成灰色船舶 footprint,並被 KPI `vesselsInPortAt` 灌進「在港船數」。
...
- `TYPE_TO_CATEGORY` 補漏列官方船型名:`'拖船':'工作'`、`'起重船':'工作'`、`'多用途工作船':'工作'`、`'貨櫃輪(有導槽)':'貨櫃'`、`'油駁船':'油品'`、`'多用途船':'散雜'`(實作時對 `khh-2026-06-19.json` + snapshot 再掃一次,補齊實際出現的漏列名)。
...
- 對 `khh-2026-06-19.json` 跑 `port:ais:refilter` 並 commit 洗淨檔(預期 551 → ~441 艘)。
