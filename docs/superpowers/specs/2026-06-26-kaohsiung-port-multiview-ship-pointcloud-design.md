...
- **`cropToContent(mask: Mask, coverFrac): Mask`** —— 用 `robustExtent` 裁到實體 bbox(非 raw min/max),移除位置/留白差異與細突出物。
...
- **`registerGrid(side: Mask, top: Mask, front: Mask, gridLong: number): GridDims`**(各遮罩已 `cropToContent` 過 → 用穩健 bbox 尺寸)
  - 以最長軸 length = `nz = gridLong` 為基準:`ny = round(gridLong × side.h / side.w)`(側視寬=length、高=height)、`nx = round(gridLong × top.h / top.w)`(俯視 寬=length、另一維=beam;依約定方向)。因為用穩健 bbox,船殼長/高比例不被吸料臂或桅杆撐歪。
...
- **純單元測試**(`test/port-view-carving.test.ts`,合成遮罩、無影像檔):
  ...
  - **雙塔幻影緩解**:合成「兩端各一高塔、front 剪影含兩塔」→ 斷言 `frontMaskMaxHeightFrac` 生效時,塔只出現在其真實站位(另一端無幻影塔)。
  - `robustExtent`:實體塊 + 1px 寬突出尖刺 → 斷言 bbox 忽略尖刺(尺寸=實體塊)。
...
- **#1 風險:雙塔幻影**。visual-hull 對「兩端各有高結構」的船(挖泥船=艏駕駛台 + 艉排料塔)會把一端的塔投影到另一端的高站位 → 幻影。**已在設計中緩解**:front 剪影只在甲板高度以下生效(`frontMaskMaxHeightFrac`),塔改由 `side × top` 盒狀包絡雕出。**仍需瀏覽器目視確認塔的位置/數量正確**;若殘留,調低 `frontMaskMaxHeightFrac` 或人工檢視。
- Visual-hull 是剪影交集 → 細吸料管/桁架/桅杆丟失(已同意 A);甲板以上結構呈**盒狀**(非精細外型),在點雲尺度可接受。
- 細突出物(桅杆/吸料臂)可能影響去背與比例 → 用 `coverFrac` 穩健 bbox + `bgTolerance` 緩解,個案微調。
- 透視截圖(非真正正投影)有輕微變形;側視最輕,可接受。
- 無真實尺度 → `lengthM:null`;執行期仍以各船 LOA 等比縮放(與既有一致;`toTemplate` 只讀 `points`,null 安全)。

## 前置條件(實作前)

- **使用者把 6 張截圖存進 `examples/kaohsiung-port/data/models/views/工程/`**(依檔名約定命名),如同遊艇要先把 glb 丟進 `data/models/`。截圖目前在對話裡、尚未落地成檔。
- 提供挖泥船**來源網址 + 授權**(見下)。
