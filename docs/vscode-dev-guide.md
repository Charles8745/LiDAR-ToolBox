...
**戰情室 app 內相機操作**:滑鼠左鍵拖曳=環繞、滾輪=縮放、右鍵拖曳=平移;鍵盤(自寫於引擎 `applyKeyboardPan`)——**↑↓←→=沿地面前後左右**(無垂直)、**空白=上升 / 左Ctrl=下降**、**按住左Shift=加速**。可調:`main.ts` 的 `keyPanSpeed: 0.4`(速度因子)、`keyPanBoost: 3`(Shift 倍率)。
