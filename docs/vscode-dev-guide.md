...
**戰情室 app 內相機操作**:滑鼠左鍵拖曳=環繞(**旋轉中心自動對到畫面中心的地面點**)、滾輪=縮放、右鍵拖曳=沿地面平移;鍵盤(自寫於引擎 `applyKeyboardPan`)——**↑↓←→=沿地面前後左右**(無垂直)、**空白=上升 / 左Ctrl=下降**、**按住左Shift=加速**。可調:`main.ts` 的 `keyPanSpeed: 0.2`(速度因子)、`keyPanBoost: 3`(Shift 倍率)。
